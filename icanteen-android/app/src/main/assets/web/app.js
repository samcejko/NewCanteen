...
async function fetchUserSettings() {
    try {
        ...
        if (!finalName || finalName.length < 3 || finalName.includes('???')) finalName = t('user_default', 'Uživatel NewCanteen');
        ...
        safeSet('sidebar-avatar', finalName !== 'Uživatel NewCanteen' ? finalName.substring(0, 2).toUpperCase() : 'NC');
        ...
        const checkUrls = (htmlStr) => {
            if (canOrder) {
                ...
                if (orderMatch && orderMatch[1]) {
                    orderUrl = orderMatch[1].replace(/&amp;/g, '&');
                } else {
                    const xhrMatch = htmlStr.match(/XHR_Send\(this,\s*'(.+?)'/);
                    if (xhrMatch && xhrMatch[1]) orderUrl = xhrMatch[1].replace(/&amp;/g, '&');
                }
            } else {
                const burzaMatch = htmlStr.match(/db\/dbProcessOrder\.jsp.+?type=(?:plusburza|minusburza|multiburza).+?'/);
                if (burzaMatch) {
                    burzaUrl = burzaMatch[0].replace(/'$/, '').replace(/&amp;/g, '&');
                } else {
                    const xhrMatch = htmlStr.match(/XHR_Send\(this,\s*'(.+?)'/);
                    if (xhrMatch && xhrMatch[1]) burzaUrl = xhrMatch[1].replace(/&amp;/g, '&');
                }
            }
        };
...
const translations = {
    cs: {
        ...
        'user_default': 'U\u017eivatel NewCanteen', 'not_specified': 'Neuvedeno',
    ...
    en: {
        ...
        'user_default': 'NewCanteen User', 'not_specified': 'Not specified',
